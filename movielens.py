...
def model(user_batch, item_batch, user_num, item_num, dim=5, device="/cpu:0"):
    with tf.device("/cpu:0"):
        with tf.variable_scope('lsi', reuse=True):
            ...
            # 创建用户偏置，论文中是bu
            w_bias_user = tf.Variable(tf.constant(0.01, shape=[user_num]))
            # 创建物品偏置，论文中是bi
            w_bias_item = tf.Variable(tf.constant(0.01, shape=[user_num]))
            # 通过用户的id取出对应的用户偏置
            bias_user = tf.nn.embedding_lookup(w_bias_user, user_batch)
            # 通过物品的id取出对应的物品偏置
            bias_item = tf.nn.embedding_lookup(w_bias_item, item_batch)
            # 创建用户隐语义向量
            w_user = tf.Variable(tf.truncated_normal(shape=[user_num, dim],stddev=0.02))
            # 创建物品隐语义向量
            w_item = tf.Variable(tf.truncated_normal(shape=[item_num, dim],stddev=0.02))
            # 通过用户的id取出对应的用户隐语义
